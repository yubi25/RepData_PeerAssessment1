...
```
- The mean is `r my_mean` and median `r my_median`
...
```
- The 5-minute interval, on average across all the days in the dataset, containing the maximum number of steps is `r my_interval` 

## Inputing missing values
```{r}
my_missing_values<-nrow(my_data[is.na(my_data$steps), ])
```
- The total number of missing values in the dataset is `r my_missing_values`
...
```
- Imputing missing data on the estimates of the total daily number of steps changes the mean (from `r my_mean` to `r my_new_mean`) and median (from `r my_median` to `r my_new_median`). 
